...
export class MessageQueue implements AsyncIterable<SDKUserMessage> {
  ...
  push(message: string): void {
    ...
    if (waiter) {
      logger.debug(
        `[MessageQueue] Found waiter! Delivering message directly: "${message}"`
      );
      waiter({
        type: "user",
        message: {
      ...
      logger.debug(
        `[MessageQueue] No waiter found. Adding to queue: "${message}"`
      );
      this.queue.push({
        type: "user",
        message: {
  ...
  close(): void {
    logger.debug(
      `[MessageQueue] close() called. Waiters: ${this.waiters.length}`
    );
    this.closed = true;
    this.closeResolve?.();
  }
